...
ThunkAction<AppState> signIn({String email, String password}) {
  return (Store<AppState> store) async {
    ...
    JwtToken token = serializers.deserializeWith(JwtToken.serializer, response);
    store.dispatch(SaveJwtAction(
        token: token, error: token != null ? token.accessToken : 'no token'));
  };
}
